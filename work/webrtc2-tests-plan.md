...
- [x] `usePeerTransport.iceRefresh.test.js` — 13 tests ✅ : rafraîchissement du credential TURN — minuteur armé sur le `credential_ttl` de la route et réécriture de `peer.options.config` à l'échéance, **rien d'armé** quand il n'y a pas de credential périssable (invité, mode statique, repli STUN), **la configuration TURN en place est conservée** quand le rafraîchissement ne rapporte rien d'exploitable (le piège : `fetchIceServers` rend le repli STUN sur route morte), reprise bornée puis abandon, plancher et plafond du délai (`setTimeout` déclenche IMMÉDIATEMENT au-delà de 2³¹-1 ms), garde d'identité sur supplantation pendant l'`await`, et mort du minuteur avec son `Peer`. ⚠️ Faux minuteurs armés AVANT le montage — l'inverse de `singleton.test.js` — et toute avance > `ICE_FETCH_TIMEOUT_MS` pendant une requête en vol la fait sortir en repli, ce qui verdit un test pour la mauvaise raison (mesuré). Six contre-épreuves consignées dans son en-tête
- [x] `peers2Store.peerRuntime.test.js` — 19 tests ✅ (store) : runtime du Peer singleton — ref-counting planchéré à 0, garde d'init (**la promesse traverse le state réactif sans être enveloppée** : identité préservée, `await` intact), compteurs de reconnexion et de rafraîchissement ICE, annulation réelle des **trois** timers avec retour booléen (⚠️ le versant store du minuteur ICE est indispensable : le retirer du vrai `resetPeerState` laisse `usePeerTransport.iceRefresh.test.js` entièrement vert, celui-ci n'exerçant que la doublure), `resetPeerState({ keepConsumerCount: true })` qui **préserve** le compteur après un échec d'init pour qu'un retry reparte d'un compte juste, et la **closure de détachement des listeners du Peer** : identité préservée elle aussi (une fonction n'est jamais proxifiée), exécutée-puis-oubliée, exécutée avant d'être remplacée, absorbée si elle jette sans interrompre le reset
