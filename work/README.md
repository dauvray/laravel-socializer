...
|---|---|---|
| [webrtc2-todo.md](webrtc2-todo.md) | ouvert | items de pérennisation du module : sémantique de `peerInitPromise`, machine à états du cycle de vie du `Peer`, trois défauts de la chaîne de présence ouverts par le bail des peerId (livré le 26/08), renommage de `usersInRoom`, observabilité. Plus un `[L]` **gelé** — déplacer le routage star dans `usePeerTransport` — qui **bloque les tâches 6 et 7** du plan de tests. |
