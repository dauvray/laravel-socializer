...
| [doc-rustines.md](doc-rustines.md) | 🟠 démarré — **3 tâches du lot 0 faites** | ~30 tâches pour que la doc cesse de compenser des défauts du code. Faites : le `CLAUDE.md` annonçait un backend non durci que le code contredit (throttle à deux buckets, 7 `validate()`, `mayReach` sur les 5 routes) ; `webrtc2/tests.md` annonçait une CI inexistante ; deux mappings PSR-4 fantômes déjà nettoyés dans `composer.json`. Reste la 4ᵉ contradiction (imports `@/` de `use-reverb-channel.md`), puis la v1 WebRTC — déclarée morte mais **encore importée par 5 composants**, sa suppression retire des annotations dans 7 fichiers. **Ne bloque pas WebRTC2** : voir sa section « ordre vis-à-vis du chantier en cours ». |
...
| [front-todo.md](front-todo.md) | **non démarré** | items front transverses, hors module. Aujourd'hui un seul : lever l'ambiguïté du nommage des directives de resize (`_horizontal` redimensionne la hauteur). |
