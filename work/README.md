...
borne qu'il laissait ouverte — rafraîchir le credential TURN avant son expiration — a été **fermée
le 25/08/2026** : la route annonce un `credential_ttl`, le transport arme un minuteur qui réécrit
`peer.options.config` avant l'échéance, et les trois arbitrages sont écrits dans
...
|---|---|---|
| [webrtc2-todo.md](webrtc2-todo.md) | ouvert | ~9 items de pérennisation : sémantique de `peerInitPromise`, peerId fantôme après `destroy()` précoce, renommage de `usersInRoom`, observabilité. Plus un `[L]` **gelé** — déplacer le routage star dans `usePeerTransport` — qui bloque deux tâches de tests. Le rafraîchissement du credential TURN en est **sorti, livré le 25/08**. |
