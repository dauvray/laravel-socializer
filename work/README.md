...
1. Le module WebRTC2 au fil de l'eau : [webrtc2-todo.md](webrtc2-todo.md),
   ...
   > déjà épinglée.
   >
   > 👉 **Par quoi reprendre : le lot F** — `CallManagerBtn` + `CallRemotePeerBtn`, les deux
   > boutons d'appel, et le dernier lot de la tâche 8. Ni l'un ni l'autre n'a de test : vérifié sur
   > les chemins d'import, `CallRemotePeerBtn` n'apparaît dans `__tests__/` que dans un
   > **commentaire** de `GroupLocalStreamBtn.permission.test.js`. Une piste déjà écrite les
   > concerne : ce fichier note que `CallRemotePeerBtn.vue:31` `inject`e **sans repli**, à
   > l'inverse de la convention du paquet.
   >
   ...
   > le 30/08. Reste le lot **F** — deux fichiers, aucun sur un chemin critique.
   >
