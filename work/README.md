...
1. Le module WebRTC2 au fil de l'eau : [webrtc2-todo.md](webrtc2-todo.md),
   ...
   >
   > 👉 **Par quoi reprendre, au 29/08 — quatre choses, dans cet ordre :**
   > 1. **Le `[S]` « la mesure qui décide de la bascule d'`enforce` n'est observable nulle part »**
   >    — passé devant le 29/08 au point d'étape QA, et c'est le seul changement d'ordre. Petit, et
   >    c'est lui qui débloque le geste de déploiement annoncé plus bas : sans lui, `enforce` reste
   >    faux **par défaut d'observation** et non par décision, donc l'usurpation intra-room reste
   >    ouverte indéfiniment. Le journal serveur est la moitié qui compte.
   > 2. **Les tâches 6 et 7 du plan de tests**, qui viennent d'être débloquées : de loin le plus gros
   >    volume écrivable immédiatement, `usePeerOrchestrator` et `useMediaBroadcast` étant les deux
   >    dernières zones non couvertes **de la logique** du module.
   > 3. **(a) du routage star** — un `[S]` de 18 lignes qui libère le dernier cas bloqué, **et** qui
   ...
   > 4. **Le 🟠 `topology: 'sfu'`** — valeur annoncée, acceptée, qui produit un contexte mort sans un
   ...
   > les deux voies de l'énoncé écartées. Il reste un geste de DÉPLOIEMENT, pas de code :
   > `SOCIALIZER_PEER_ATTESTATION_ENFORCE=true` une fois que `uncorroboratedAdmissions` cesse de
   > bouger en usage nominal — **ce que rien ne permet de constater aujourd'hui**, d'où le point 1.
   >
