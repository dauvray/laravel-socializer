...
| [front-todo.md](front-todo.md) | **non démarré** | un item : le ping d'ouverture de session part avant que pusher n'ait confirmé l'abonnement, et Reverb le rejette — l'utilisateur peut rester hors ligne deux minutes. Le correctif naïf (`subscribed(cb)`) ne part jamais sur un canal déjà confirmé. |
