...
1. Le module WebRTC2 au fil de l'eau : [webrtc2-todo.md](webrtc2-todo.md),
   ...
   >
   > 👉 **Par quoi reprendre : le lot D** — `useMediaControls` + les contrôles de
   > `MediaBroadcastPlayer`, son seul appelant. Il arrive avec ses deux graines déjà trouvées
   > (`isFullscreen`/`isPip` que personne ne lit et qu'aucun listener ne met à jour ; la sentinelle
   > `null` atteignable par un slot `#video`) et son coût connu : happy-dom n'a **aucun** des quatre
   > membres de `document` du plein écran et du PiP, mesuré.
   >
   ...
   > le 30/08. Restent les lots **D, E et F** — cinq fichiers, désormais nommés (« lots C à F » ne
   > désignait rien), aucun sur un chemin critique.
   >
