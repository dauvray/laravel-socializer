...
|---|---|---|
| [webrtc2-todo.md](webrtc2-todo.md) | ouvert | items de pérennisation du module : sémantique de `peerInitPromise`, machine à états du cycle de vie du `Peer`, ce qui reste de la chaîne de présence ouverte par le bail des peerId — le verrou coalescent et la purge d'une room qui se vide sont fermés le 27/08, restent le diff aveugle à un départ+retour, la fraîcheur de `roomMembers` et le client star qui compose un hub absent — renommage de `usersInRoom`, observabilité. Plus un `[L]` **gelé** — déplacer le routage star dans `usePeerTransport` — qui **bloque les tâches 6 et 7** du plan de tests. |
