...
1. Le module WebRTC2 au fil de l'eau : [webrtc2-todo.md](webrtc2-todo.md),
   ...
   >
   > ✅ **Le lot E est FAIT le 31/08** — `LocalMediaPlayer` + `MediaBroadcastProvider`, en **trois**
   > fichiers et non deux : l'énoncé se trompait de périmètre sur deux points, et le troisième
   > fichier est ce qui ferme le joint. 25 cas, 23 contrôles mesurés, l'étage passe de 10 à 12
   > fichiers exercés sur 15.
   >
   > ⭐ **Deux 0 croisés valent le lot, un par joint.** Permuter la clé de `provide` rougit 1 cas et
   > **0 dans les deux autres fichiers** — `WEBRTC_API_KEY` n'a qu'un `provide` et qu'un `inject`
   > dans tout le dépôt, donc aucun étage ne voit les deux bouts. Et reconstruire
   > `screenStreamData.stream` en copie rougit 1 cas **et 0 ailleurs** : mon écran partagé peut
   > disparaître dès que je coupe ma caméra sans qu'un cas d'étage bouge.
   >
   > ⚠️ **La sortie B a rendu plus que le retrait.** Le `v-bind="$attrs"` redondant **désarmait le
   > contrôle du voisin** : tant qu'il était là, `inheritAttrs: false` rougissait 0 cas ici contre 1
   > chez le jumeau. D'où la règle neuve — **un contrôle à 0 doit faire chercher quelle AUTRE ligne
   > absorbe la mutation** avant de conclure que le test est inutile.
   >
   > ⚠️ **Deux contre-épreuves ont rougi ZÉRO cas, les deux fois par faute du test** — quatrième
   > lot consécutif. Cette fois les deux fautes sont des faits de harnais réutilisables :
   > **`wrapper.vm` traverse `defineExpose`** (un test d'exposition écrit ainsi ne peut pas
   > échouer — il faut une ref de template), et **pousser dans un tableau NU n'est vu par aucun
   > watcher**, profond ou pas.
   >
   > 🟠 **Un défaut latent en est sorti, mesuré** : le watch de `props.users` n'étant pas profond,
   > la présence de TOUS les providers dépend du fait qu'`useReverbChannel` réaffecte son tableau.
   > Y écrire un `push` rougit **0 cas sur 1417** et arrête la synchronisation en silence
   > (fail-closed). Item ouvert dans [webrtc2-todo.md](webrtc2-todo.md) ; la moitié provider est
   > déjà épinglée.
   >
   > ✅ **Le lot F est FAIT le 31/08, et la TÂCHE 8 EST FERMÉE** — l'étage `Widgets/**` passe à 14
   > couverts sur 15, le 15ᵉ étant `SpectrumAnalyzer`, exclu et assumé. **Ce n'était pas un lot de
   > tests.** Son énoncé faisait six mots ; il a rendu 56 cas, **8 corrections de production** et
   > six affirmations réfutées.
   >
   > 🔴 **Le défaut qu'il a fermé ne concernait pas les boutons.** Sur le chemin « aucun peerId
   > local publiable », la FSM restait en CALLING **pour la vie de l'onglet** : spinner sans bouton
   > pour raccrocher, et surtout **plus aucun appel possible, vers personne** — `transition(CALLING)`
   > refusant de partir de `calling`. C'était la **troisième route** de la famille « FSM bloquée en
   > calling », dont les deux autres étaient fermées et documentées depuis le 29/08. Et le
   > commentaire de `usePeerCore` affirmait « l'utilisateur peut rappeler ».
   >
   > 🔴 **Second défaut, hors périmètre et corrigé quand même : un appel « vocal » allumait et
   > TRANSMETTAIT la caméra.** `_enterCallSession` passait `startCurrentStream(true)` à une fonction
   > qui ne prenait aucun paramètre — l'argument était ignoré depuis toujours, et les contraintes
   > lisaient `isVideoEnabled`, vrai par défaut.
   >
   > ⭐ **Le chiffre qui vaut le lot : sept 0 croisés qui imposent un TROISIÈME fichier de test.**
   > Renommer un attribut de la couture, couper une prop, croiser les écouteurs — chacune de ces
   > mutations rougit le fichier du joint et **0 cas** de celui de la barre. Plus deux autres 0
   > croisés, un par correction de fond : chaque moitié n'est visible que dans son propre fichier,
   > donc **corriger une seule aurait laissé le défaut vivant, suite verte à l'appui**.
   >
   > ⚠️ **Première fois en six lots qu'aucun contrôle à 0 n'est une faute du test.** Trois zéros :
   > un assumé, et **deux par absorption** — re-mesurés en neutralisant les DEUX mécanismes
   > ensemble, rouges alors. C'est la règle du lot E qui a évité de supprimer deux lignes utiles.
   >
   > 👉 **Par quoi reprendre : plus rien dans ce fichier n'est prioritaire.** Le plan de tests est
   > clos. Les candidats sont [doc-rustines.md](doc-rustines.md) (lot 1, la v1 morte encore importée
   > par cinq composants — et le lot F vient d'y trancher la décision sur `webrtc2Events.js` :
   > supprimer), ou les items ouverts de [webrtc2-todo.md](webrtc2-todo.md), dont **aucun n'est 🔴**.
   >
   ...
   >
   > ✅ **La tâche 8 était la dernière ouverte au plan de tests, et elle est fermée.** L'énoncé
   > annonçait « deux fichiers, aucun sur un chemin critique » : c'était faux sur le second point,
   > et c'est la leçon la plus utile de ce lot. Le chemin critique n'était pas dans les boutons —
   > il était dans ce que leur couverture a obligé à regarder.
   >
