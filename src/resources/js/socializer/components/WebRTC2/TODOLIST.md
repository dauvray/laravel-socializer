...
- [ ] **Tests unitaires** `[XL]` : couplage fort à Vue/inject/eventBus rend le code non-testable → extraire logique pure. Débloqué pour les couches extraites (`useConnectionPool` 31 tests, `useCallManager` 45 tests) et pour le pool de players (`usePeerMedia.players` 15 tests) ; reste `usePeerConnections`, le reste de `usePeerMedia` (streams), `createPeerContext`
