...
<script>
    ...
    import streamPeerCallback from '~socializer/callbacks/vocalPlayerCallback.js'
    ...
    export default {
        ...
        methods: {
            ...
            updateStreams() {
                if (!this.onAirRoom || !this.remoteStreams?.[this.onAirRoom]) {
                    this.streams = []
                    return
                }
                ...
                Object.keys(this.remoteStreams[this.onAirRoom]).forEach(slug => {
                    const userStreams = this.remoteStreams[this.onAirRoom]?.[slug]?.[streamType]
