...
| **Feed** | `Feed/` | Fil d'actualité : `Feed.vue` (⚠️ encore en Options API, mais son câblage Reverb est dans un `setup()` — [pourquoi](../reference/use-reverb-channel.md#un-whisper-de-départ-senregistre-avant-le-composable)), `PostList.vue`, `Post.vue`, `widgets/ShareButton.vue`, `SharedThumbnail.vue`. Fan-out serveur par les Jobs `SendPostToFollowers`. |
