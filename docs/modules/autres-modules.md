...
| **Server** | `Server/` | « Serveurs » façon Discord : `Server.vue`, `Room.vue`, `RoomHeader.vue`, `roomSettings.js`, `widgets/` (ServerList, RoomSidebar, RoomUsersList, LockedRoom, SettingsModal). Les presets de rooms viennent de `src/config/modules.php`. **La navigation serveur → salon → contenu a sa doc : [serveurs-et-salons.md](serveurs-et-salons.md)** — deux gardes y ont un mode de panne totalement silencieux. |
...
| **ClassRoom** | `ClassRoom/` | Salle de classe : `ClassRoomComponent.vue`, `ConfigPanel.vue`. Imbrique `ChatComponent` et `WhiteboardComponent`, et porte son propre canal data WebRTC2 (`room.id`) pour propager les bascules d'affichage — donc **deux contextes** dans le même onglet, le sien et celui du tableau. ⚠️ **En topologie MESH**, comme les deux autres consommateurs du canal data : il ne passe pas d'`options`. *(Cette ligne a annoncé le contraire — « cas d'usage type de la topologie star » — jusqu'au 01/09/2026. C'était faux depuis toujours : la v1 qu'il utilisait n'avait aucune notion de topologie. Corrigé en migrant le module, parce qu'un objet `options` passé pour obtenir `star` **efface le défaut en bloc** et fait disparaître `topology`.)* |
