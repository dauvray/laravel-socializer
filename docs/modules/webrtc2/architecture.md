...
| `peerStore.localPeerAttestation` (ce que le serveur a signé pour MOI) | `usePeerTransport` seul — posée dans `_doInit` **avant `new Peer`** (le peerId est choisi par nous, donc attestable en parallèle de l'ICE), renouvelée par `_refreshAttestation`, oubliée par `resetPeerState`. ⚠️ La demander à l'`'open'` rouvrirait une fenêtre : le chemin « bail encore valide » de `useConnectionPool` connecte dès que `waitForMeReady` rend la main | un seul lecteur : `usePeerConnections._buildPeerConnectionConfig`, site unique de construction de la `metadata` |
