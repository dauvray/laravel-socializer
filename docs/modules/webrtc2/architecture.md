...
| `peerStore.attestedPeers[peerId]` — **qui est en face, jamais qui a le droit d'entrer** | `usePeerTransport._attestedSlugFor`, seul écrivain, sur le verdict rendu par `POST /verify-peer-attestation`. Mémoïse aussi les REFUS (`{ slug: null }`), sinon un pair refusé qui insiste ferait payer un aller-retour à chacune de ses tentatives. ⚠️ Indexé par **peerId** et non par slug, à l'inverse de `remotePeersId` : le fait décrit est « CETTE identité PeerJS appartient à untel », et le peerId est la clé que le récepteur tient en main (`conn.peer`). ⚠️ Survit à `resetPeerState` — l'identité des pairs distants ne dépend pas de la vie de MON `Peer` (même arbitrage que `contextRegistry`) ; ce qui la périme est son propre `expiresAt` | **`peerStore.getAttestedPeer(peerId)`, et deux lecteurs seulement** : `_resolveSenderSlugFromIncomingConn` (résolution inverse) et la corroboration de `_isAuthorizedIncomingPeer`. ⚠️⚠️ **JAMAIS une allowlist.** Le verser dans `remotePeersId` ferait d'un pair attesté un « interlocuteur d'appel direct vérifié » sans qu'aucun appel n'ait été autorisé — l'auto-inscription que `authorizedCallPeers` a fermée, par une autre porte. Vérifié au grep par `peers2Store.attestedPeers.test.js` |
| `peerStore.localPeerAttestation` (ce que le serveur a signé pour MOI) | `usePeerTransport` seul — posée dans `_doInit` **avant `new Peer`** (le peerId est choisi par nous, donc attestable en parallèle de l'ICE), renouvelée par `_refreshAttestation`, oubliée par `resetPeerState`. ⚠️ La demander à l'`'open'` rouvrirait une fenêtre : le chemin « bail encore valide » de `useConnectionPool` connecte dès que `waitForMeReady` rend la main | **un seul lecteur de la VALEUR** : `usePeerConnections._buildPeerConnectionConfig`, site unique de construction de la `metadata`. `Widgets/UI/Report/Debug.vue` en lit la **présence** (`? 'présente' : 'absente'`), jamais la chaîne — ⚠️ c'est une identité signée valable jusqu'à son échéance, et le panneau est fait pour être montré pendant un diagnostic ; `Debug.attestation.test.js` garde la distinction |
...
- **La composition n'appartient qu'à un contexte VIVANT et DÉTENTEUR de son id.** Deux règles, deux
  mécanismes, et elles ne se remplacent pas : *(1)* un contexte en arrêt n'écrit pas la composition
  — `getRoomUsersDiff` teste `ctx.isShuttingDown` juste après la barrière `waitForMeReady`, dont
  l'`effectScope` est détaché et survit donc au démontage ; sans ce garde, un tour en vol
  **ressuscite** l'entrée que `destroy()` vient de retirer, et plus rien ne la retire jamais.
  *(2)* seul le détenteur enregistré efface — `clearRoomMembers(contextId, owner)`, jumeau du garde
  d'identité de `unregisterContext` ; sans lui, le démontage d'un contexte emporte l'allowlist de
  son homonyme vivant, qui refuse alors toute connexion entrante du chemin (a) en silence.
  ⚠️ Distinct du drain de `syncUsersConnections`, qui lit le même drapeau pour décider du tour
  **suivant** (« le tour déjà commencé va jusqu'au bout ») : la règle (1) porte sur la seule
  écriture que le tour **en cours** ne doit plus faire. Le raisonnement complet, les trois pistes
  écartées et la fenêtre assumée sont dans [securite.md](securite.md)
